...
// GET request to 'api/users/<username>/notes'.
// Respond with the full data for all of the notes
//   created by the user.
// Find the user's notes by using the username in the url.
func getNotes(w http.ResponseWriter, r *http.Request, username string) (error, int) {
	// The url's prefix directs the http request to this function.
	// Trim this redundant prefix off of the url.
	baseLocation := concat("/api/users/", username, "/notes")
	...
	js, err := json.Marshal(user.Notes)
	if err != nil {
		return err,
...
// GET request to 'api/users/<username>/note/<note-id>'.
// Respond with the full data for this note
// Find the user's notes by using the username in the url.
func getNote(w http.ResponseWriter, r *http.Request, username string, noteId string) (error, int) {
	// Get user info from the database, using the token in the header.
	user := User{}
	err := usersCollection.Find(bson.M{"username": username}).One(&user)
	if err != nil {
