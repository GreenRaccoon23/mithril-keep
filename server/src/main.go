...
func main() {
	...
	tokenSecretEnvVar := "MITHRIL_KEEP_TOKEN_SECRET"
	tokenSecretString := os.Getenv(tokenSecretEnvVar)
	if tokenSecretString == "" {
		log.Fatal(concat(tokenSecretEnvVar, " environment variable must be specified"))
	}
